...
/*	function zarfiat($inp)
	...
	function loadZarfiat($inp)
	{
		...
		$out = "<u><span style=\"color:$color;cursor:pointer;\" ".(((int)$_SESSION['customer_typ']==2)?" onclick=\"wopen('zarfiat.php?parvaz_det_id=$inp&','',600,400);\"":"").">$zarfiat</span></u>";
		return $out;
	...
	function loadParvazRangs()
	{
		//$out = "var par_array = {";
		$out = "";
		mysql_class::ex_sql("select `id`,`rang` from `parvaz`",$q);
		while($r = mysql_fetch_array($q))
		{
			if($r["rang"] != "")
				$out .= $r["id"].":\"".$r["rang"]."\",";
	...
	if($whereClause1 != "")
	{
		$whereClause .= " and `en`='1' and  parvaz_id in (select id from `parvaz` where $whereClause1)";
	}
...
*/
	...
	switch ($_SESSION["customer_typ"])
	{
		...
		case 1 : 
			...
			$grid->columnHeaders[14]="کمیسیون<br>(درصد)";
			$grid->columnFunctions[14] = "poorsant1";
			$grid->canDelete = FALSE;
		break;
		case 2 :
...
//                        $grid->columnHeaders[13] = null;
	...
	{
	        $grid->addFeild("id",12);
        	$grid->columnHeaders[12] = "ذخیره";
	        $grid->columnFunctions[12] = "loadCustomerZakhire";
        	$grid->columnAccesses[12] = 0;
		$grid->addFeild("poor_def",13);
		$grid->columnHeaders[13] = "کمیسیون<br/>پایه(٪)";
		$grid->columnFunctions[13] = "zarfiat1";
		$grid->columnAccesses[13] = 1;
		$grid->columnJavaScript[13] = "onkeyup=\"findOthers(this);\"";
		$grid->addFeild("id",14);
		$grid->columnHeaders[14]="کمیسیون<br/>درصد";
		$grid->columnFunctions[14] = "poorsant";
	}
	if((int)$_SESSION["customer_typ"]==2)
		$grid->columnAccesses[6] = 1;
