<?php
	...
	{
		$su = sanadJam("`typ`=-1 and `sanad_typ`=0 and (date(`tarikh`) >= '$aztarikh' and date(`tarikh`) <= '$tatarikh') ".(($customer_id>0)?" and `customer_id` = $customer_id ":''));
	...
	$xgrid->whereClause[$gname] = "`typ`=-1 and `sanad_typ`=0 and (date(`tarikh`) >= '$aztarikh' and date(`tarikh`) <= '$tatarikh') ".(($customer_id>0)?" and `customer_id` = $customer_id ":'');
...
<script type="text/javascript" >
	...
	{
		$("#jam").html('');
		$("#jam").load("sales_admin_sanad.php?customer_id="+$("#customer_id").val()+"&aztarikh="+$("#aztarikh").val()+"&tatarikh="+$("#tatarikh").val()+"&jam=true&r="+Math.random()+"&");
	}
