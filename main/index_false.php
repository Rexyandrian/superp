...
//	 die("<script>window.location='login.php';</script>");
	...
	function selectParvaz($inp)
	{
		...
		$time_new = $saat;
		mysql_class::ex_sql("select `tarikh`,`saat` from `parvaz_det` where `id`=$inp",$q);
		...
		$out = '&nbsp;';
		if( ($tarikh>$today) || ($tarikh==$today && $saat>$time_new) )
...
/*	function zarfiat($inp)
	...
	function parvaz_detail($inp)
	{
		...
		if($out==0)	
			$out ='0';
		$out .="&nbsp;<u><span style=\"color:firebrick;cursor:pointer;\" onclick=\"wopen('parvaz_forookhte.php?parvaz_det_id=$inp&','',600,500);\"> جزئیات</span></u>";
		return $out;
	}
	if($firstVisit){
		//echo "+++++++first+++++++";
...
*/
	...
	$grid->columnFunctions[4] = "zarfiat1";
	...
	switch ($_SESSION["customer_typ"])
	{
		case 0 : 
			...
			$grid->columnFunctions[4] = "zarfiat";
		...
		case 1 : 
			...
			$grid->columnHeaders[10]="کمیسیون<br>(درصد)";
			$grid->columnFunctions[10] = "poorsant1";
			$grid->canDelete = FALSE;
		break;
		case 2 : 
			...
			$grid->columnFunctions[7] = "zarfiat1";
			$grid->columnHeaders[9] = "کمیسون<br> پایه(درصد)";
			$grid->columnFunctions[9] = "zarfiat1";
			$grid->addFeild("id");
			$grid->addFeild("id");
                        $grid->columnAccesses[count($grid->columnHeaders)-1] = 0;
			$grid->columnHeaders[10]="کمیسیون <br>درصد";
			$grid->columnFunctions[10] = "poorsant";
			$grid->columnHeaders[11]="جزئیات فروش";
			$grid->columnFunctions[11] = "parvaz_detail";
			$grid->columnAccesses[3] = 1;
			$grid->columnAccesses[4] = 1;
   ...
   <div id="header" style="display:none;" style="background: #D2B48C;">
    ...
    <div id="main" style="background: #D2B48C;"><div id="main2" style="background: #D2B48C;">	
		...
		<form method="POST" id="cust_frm" >
                <h2 align="center"><b> <?php echo lang_fa_class::title." - ".lang_fa_class::all_manegment; ?></b>
		...
		<tr>
		      <td colspan="2" align="center" style="background: #F6A255;" >
			    <form id="search_form" method="get" action="index1.php" >
				<table style="background: #F6A255;color:#000000;">
					<tr>
						<td>
							مبدأ :
						</td>
			...
			<form id="reserve_frm" >	
				<table >
					...
					<tr>
						...
						<td>
							<input class="inp1"  value="رزرو موقت" class="inp" id="reserve" type="button" style="width:auto;font-weight:bold;">
			...
			<table width="100%" border="1">
				<tr>
	                		<td align="center" width="100%"   >
                                       		<iframe id="main_con" name="main_con" height="900px" scrolling="yes" bordercolor="Green" align="center" width="100%" height="100%" src="index1.php" style="border-style:solid;border-width:1px;border-color:White">						
					</iframe>
                                        </td> 
	                	</tr>
			</table>
