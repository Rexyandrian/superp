...
*/
		...
		public function __construct($tableName,$gridName,$dataArray=null){
			...
			$this->mysql_class->enableCache = FALSE;
			...
			if($this->class_enabled){
				if($this->query == "" && !$this->loadArray){
					$this->mysql_class->directQuery("select * from $tableName where 0=1",$q);
				}else{
					if(!$this->loadArray)
						$this->mysql_class->directQuery($this->query,$q);
					$this->canEdit = FALSE;
					$this->canAdd = FALSE;
		...
		public function intial(){
			...
			if(isset($_REQUEST["ajax_post_back_".$this->gridName]) && (int)$_REQUEST["ajax_post_back_".$this->gridName]==1 && !isset($_REQUEST['mod_'.$this->gridName]))
			{
				...
				else
				{
					...
					$this->mysql_class->directQueryx("update `".$this->tableName."` set `$field`='$valu' where id=$selectedId");
					echo 58;
				}
				exit();
			}			
				...
				switch($mod){
					case "edit":
						...
						if($this->editFunction!=null){
							...
							$this->mysql_class->directQueryx("update `".$this->tableName."` set `".$tmp[2]."`=".(($this->field_is_string($tmp[2]))?"'":"").$valu.(($this->field_is_string($tmp[2]))?"'":"")." where `".$this->tableName."`.`id`='$selectedId'");
						}
                                                for($i=0;$i<count($this->fieldList);$i++){
					...
					case "add":
						if($this->addFunction!=null){
							...
							$this->mysql_class->directQueryx("insert into `".$this->tableName."` ".$qur);
						}
                                                for($i=0;$i<count($this->fieldList);$i++){
                                                        if($this->columnFilters[$i]!==FALSE)
                                                        {
					...
					case "delete":
						if($this->deleteFunction!=null){
							...
							$this->mysql_class->directQueryx("delete from `".$this->tableName."` where `".$this->tableName."`.`id`=$selectedId");
						}
                                                for($i=0;$i<count($this->fieldList);$i++){
                                                        if($this->columnFilters[$i]!==FALSE)
                                                        {
					...
					case "group":
						...
						else
						{
							...
							if($tmp!="")
							{
								...
								$tmp = " where $tmp";
								$this->mysql_class->directQueryx("delete from `".$this->tableName."` $tmp");
							}
						}
						break;
					case "refreshcombo":
...
//								$filter .= " `".$this->fieldList[$i]."` like '%".$callbackfn($this->columnFilters[$i])."%' and";
				...
				if($this->query == "" && !$this->loadArray){
					$this->mysql_class->directQuery('select '.$this->arrayToString($this->fieldList).' from `'.$this->tableName.'` '.$this->extraClause.' where '.$filter.$this->whereClause,$this->dataSet);
				}else if(!$this->loadArray){
					$this->mysql_class->directQuery($this->query,$this->dataSet);
				}
				if(!$this->loadArray){
				...
				if($this->query == "" && !$this->loadArray){
					$this->mysql_class->directQuery("select ".$this->arrayToString($this->fieldList)." from ".$this->tableName." ".$this->extraClause." where $filter ".$this->whereClause." limit ".$this->pageNumber.",".$this->pageCount,$this->dataSet);
				}else if(!$this->loadArray){
					$this->mysql_class->directQuery($this->query." limit ".$this->pageNumber.",".$this->pageCount,$this->dataSet);
				}
			}	
		}
...
*/				if($this->canDelete){
			...
			$out .= "<input type=\"button\" class=\"" . $this->cssClass . "_insert_button\" value=\"".$this->nextButton."\" onclick=\"nextPage_".$this->gridName."();\" ".((($this->rowCount/$this->pageCount)<=1)?"disabled=\"disabled\"":"")."/>";
